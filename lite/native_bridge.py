...
# WINFUNCTYPE (stdcall) only exists on Windows. On macOS/Linux the native DLL is
# never loaded — the app uses the pure-Python fallbacks — but this module and the
# HotkeyCallback type must still import cleanly, so fall back to CFUNCTYPE there.
_FUNCTYPE = getattr(ctypes, "WINFUNCTYPE", ctypes.CFUNCTYPE)
HotkeyCallback = _FUNCTYPE(None, c_int, c_void_p)
